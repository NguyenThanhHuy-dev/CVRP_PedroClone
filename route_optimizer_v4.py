        ...
        for v in range(2):
            for i in range(self.n):
                for j in range(self.n):
                    if i != j and self.local_dist[i, j] > 0:
                        self.wcnf.append(
                            [-self.conNet[v][i][j]], weight=self.local_dist[i, j]
                        )
...
def relocate_search(
    ...
    Sử dụng Soft Capacity Constraint: Chấp nhận vượt tải nhưng phạt rất nặng.
    """
    best_saving = 0.0  # Chỉ chấp nhận các nước đi tạo ra saving > 0 (cải thiện)
    best_move = None
    ...
    for r1 in route_ids:
        for pos1, customer in enumerate(routes[r1]):
            # 1. Tính chi phí dỡ khách hàng khỏi r1 (Distance)
            route1 = routes[r1]
            if len(route1) == 1:
            ...
            # 2. Tính mức phạt vượt tải của r1 (trước và sau khi dỡ)
            d1_old = route_demands[r1]
            overload_r1_old = max(0, d1_old - capacity)
            overload_r1_new = max(0, d1_old - demands[customer] - capacity)

            # 3. Thử chèn vào các tuyến khác
            for r2 in route_ids:
                if r1 == r2:
                    continue
                ...
                overload_r2_new = max(0, d2_old + demands[customer] - capacity)
                ...
                # Tìm vị trí chèn tốt nhất vào r2
                for pos2 in range(len(route2) + 1):
                    prev2 = route2[pos2 - 1] if pos2 > 0 else 0
                    next2 = route2[pos2] if pos2 < len(route2) else 0

                    insert_cost = (
                        distances[prev2, customer]
                        + distances[customer, next2]
                        - distances[prev2, next2]
                    )

                    # TỔNG TIẾT KIỆM = Tiết kiệm quãng đường + Tiết kiệm tiền phạt
                    distance_saving = remove_cost - insert_cost
                    total_saving = distance_saving + penalty_saving
    ...
    for r1, r2 in combinations(route_ids, 2):
        for pos1, c1 in enumerate(routes[r1]):
            for pos2, c2 in enumerate(routes[r2]):
                ...
                d2_new = d2_old - demands[c2] + demands[c1]
                ...
                distance_saving = (old_cost1 + old_cost2) - (new_cost1 + new_cost2)
                total_saving = distance_saving + penalty_saving
...
def inter_route_local_search(
    ...
    while improved and iteration < max_iterations:
        ...
        # Try relocate
        routes, saving, did_improve = relocate_search(
            routes, distances, demands, capacity
        )
        if did_improve:
            total_saving += saving
            improved = True
        ...
        # Try exchange
        routes, saving, did_improve = exchange_search(
            routes, distances, demands, capacity
        )
        if did_improve:
            total_saving += saving
        ...
        routes, saving, did_improve = or_opt_search(
            routes, distances, demands, capacity, seq_len=2
        )
        if did_improve:
            total_saving += saving
            improved = True
        ...
        routes, saving, did_improve = or_opt_search(
            routes, distances, demands, capacity, seq_len=3
        )
        if did_improve:
            total_saving += saving
            improved = True
        ...
        # Try Cross-Exchange
        routes, saving, did_improve = cross_exchange(
            routes, distances, demands, capacity
        )
        if did_improve:
            total_saving += saving
...
def calculate_augmented_cost(
    route: List[int], distances: np.ndarray, penalties: np.ndarray, lambda_param: float
) -> float:
    ...
    for i in range(len(route) - 1):
        cost += (
            ...
            + lambda_param * penalties[route[i], route[i + 1]]
        )
    ...
    while improved and iteration < max_iterations:
        ...
        for r1 in route_ids:
            for pos1, customer in enumerate(routes[r1]):
                ...
                old_aug = (
                    distances[prev1, customer]
                    + lambda_param * penalties[prev1, customer]
                    + distances[customer, next1]
                    + lambda_param * penalties[customer, next1]
                )
                new_aug = (
                    distances[prev1, next1] + lambda_param * penalties[prev1, next1]
                )
                ...
                for r2 in route_ids:
                    ...
                    for pos2 in range(len(route2) + 1):
                        ...
                        old_aug2 = (
                            distances[prev2, next2]
                            + lambda_param * penalties[prev2, next2]
                        )
                        new_aug2 = (
                            distances[prev2, customer]
                            + lambda_param * penalties[prev2, customer]
                            + distances[customer, next2]
                            + lambda_param * penalties[customer, next2]
                        )
        ...
        for r1, r2 in combinations(route_ids, 2):
            for pos1, c1 in enumerate(routes[r1]):
                for pos2, c2 in enumerate(routes[r2]):
                    demand_r1 = (
                        sum(demands[c] for c in routes[r1]) - demands[c1] + demands[c2]
                    )
                    demand_r2 = (
                        sum(demands[c] for c in routes[r2]) - demands[c2] + demands[c1]
                    )
                    ...
                    old_aug = (
                        distances[prev1, c1]
                        + lambda_param * penalties[prev1, c1]
                        + distances[c1, next1]
                        + lambda_param * penalties[c1, next1]
                        + distances[prev2, c2]
                        + lambda_param * penalties[prev2, c2]
                        + distances[c2, next2]
                        + lambda_param * penalties[c2, next2]
                    )
                    new_aug = (
                        distances[prev1, c2]
                        + lambda_param * penalties[prev1, c2]
                        + distances[c2, next1]
                        + lambda_param * penalties[c2, next1]
                        + distances[prev2, c1]
                        + lambda_param * penalties[prev2, c1]
                        + distances[c1, next2]
                        + lambda_param * penalties[c1, next2]
                    )
        ...
        for r_id, route in routes.items():
            ...
            for i in range(len(route) - 1):
                for j in range(i + 2, len(route)):
                    ...
                    old_aug = (
                        distances[prev_i, route[i]]
                        + lambda_param * penalties[prev_i, route[i]]
                        + distances[route[j], next_j]
                        + lambda_param * penalties[route[j], next_j]
                    )
                    new_aug = (
                        distances[prev_i, route[j]]
                        + lambda_param * penalties[prev_i, route[j]]
                        + distances[route[i], next_j]
                        + lambda_param * penalties[route[i], next_j]
                    )
...
def guided_local_search(
    ...
    for iteration in range(max_iterations):
        ...
        new_routes, new_cost = gls_local_search(
            current_routes, distances, demands, capacity, penalties, lambda_param
        )
        ...
        if new_cost < best_cost:
            print(
                f"  Iteration {iteration + 1}: {best_cost} -> {new_cost} (IMPROVED -{best_cost - new_cost})"
            )
...
def lns_perturbation(
    routes: Dict[int, List[int]],
    distances: np.ndarray,
    demands: np.ndarray,
    capacity: int,
    destroy_pct: float = 0.3,
    destroy_method: str = "random",
) -> Dict[int, List[int]]:
    """
    ...
    if destroy_method == "worst":
        ...
        for c in all_customers:
            ...
            cost_saved = (
                distances[prev_node, c]
                + distances[c, next_node]
...
def intra_route_2opt(
    routes: Dict[int, List[int]], distances: np.ndarray
    ...
    if best_move:
        ...
        # Reverse segment [i, j]
        new_routes[r_id] = (
            routes[r_id][:i] + routes[r_id][i : j + 1][::-1] + routes[r_id][j + 1 :]
        )
        return new_routes, best_saving, True
...
def cross_exchange(
    ...
    Đã nâng cấp Soft Capacity Constraint.
    """
    best_saving = 0.0
    best_move = None
    PENALTY_RATE = 10000.0
            ...
            for len2 in range(1, min(len(route2) + 1, max_seg_len + 1)):
                for i in range(len(route1) - len1 + 1):
                    for j in range(len(route2) - len2 + 1):
                        ...
                        old_cost = (
                            distances[prev1, seg1[0]]
                            + distances[seg1[-1], next1]
                            + distances[prev2, seg2[0]]
                            + distances[seg2[-1], next2]
                        )
                        new_cost = (
                            distances[prev1, seg2[0]]
                            + distances[seg2[-1], next1]
                            + distances[prev2, seg1[0]]
                            + distances[seg1[-1], next2]
                        )

                        distance_saving = old_cost - new_cost
                        total_saving = distance_saving + penalty_saving
...
def pairwise_optimize(
    ...
    while improved and iteration < max_iterations:
        if time.time() - global_start > MAX_STEP_TIME:
            print(
                "  [TIMEOUT] Pair-wise step took too long. Skipping remaining iterations."
            )
            break
...
# Step 1: Clarke-Wright
    ...
    # Step 2: Two-Opt improvement (CHẠY TRÊN BẢN GỐC CỦA CLARKE-WRIGHT)
    if verbose:
    ...
    # CHUYỂN ĐỔI SANG ĐỊNH DẠNG DICTIONARY (SAU KHI ĐÃ TWO-OPT)
    routes = {
    ...
    if len(routes) > n_vehicles:
        print(f"\nRepairing routes: {len(routes)} -> {n_vehicles}")
        routes = route_reduction_repair(
            routes,
            demands=demands,
        ...
        # Làm sạch các tuyến rỗng nếu có
        routes = {i: r for i, r in enumerate(r for r in routes.values() if r)}
    ...
    opt_routes, opt_cost = optimize_all_routes(
        gls_routes, cvrp.distances, demands, cvrp.capacity
    )
    ...
    if verbose:
        total_time = (
            cw_time + two_opt_time + ils_time + gls_time + single_time + pair_time
        )
        ...
        print(
            f"  Vehicles:         {final_k} / {n_vehicles} ({'VALID' if is_valid else 'INVALID'})"
        )
...
def route_reduction_repair(routes, demands, capacity, distances, k):
    routes_list = [r for r in routes.values() if len(r) > 0]

    while len(routes_list) > k:
        routes_list.sort(key=lambda r: sum(demands[c] for c in r))
        smallest_route = routes_list.pop(0)

        unassigned_customers = list(smallest_route)
        ...
        for customer in unassigned_customers:
            ...
            for i, r in enumerate(routes_list):
                # BỎ ĐIỀU KIỆN HARD CONSTRAINT Ở ĐÂY: Mặc kệ capacity, cứ tính khoảng cách
            ...
            # Bắt buộc nhét vào route tốt nhất tìm được (chấp nhận vượt capacity)
            routes_list[best_route_idx].insert(best_pos, customer)
