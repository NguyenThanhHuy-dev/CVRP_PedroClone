...
# --- LOGGING ---
_LOG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logs", "gurobi")
os.makedirs(_LOG_DIR, exist_ok=True)
log_filename = os.path.join(_LOG_DIR, f"solver_gurobi_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
class AdvancedCVRPOptimizer:
    _POOL_CLEANUP_BUFFER = 2.0  # giây dành cho cleanup sau mỗi lần gọi Gurobi
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # PHASE 4: GUROBI MIP (cấu trúc y hệt run_phase_4_maxsat trong pysat)
    # ──────────────────────────────────────────────────────────────────────────
    def run_phase_4_gurobi(
        self, routes: Dict[int, List[int]]
    ) -> Tuple[Dict[int, List[int]], int]:
        """
        CHẶNG 4: TỐI ƯU CHÍNH XÁC GUROBI MIP
        ...
        logging.info(
            f"=== BẮT ĐẦU CHẶNG 4: GUROBI MIP CHÍNH XÁC | Cost: {best_cost} ==="
        )
        ...
        if single_imp_total > 0:
            best_cost = self.compute_total_cost(best_routes)
            logging.info(
                f"    -> Single Gurobi cải thiện: {single_imp_total}. Cost mới: {best_cost}"
            )
    ...
    def optimize(
        ...
        routes, best_cost = self.run_phase_4_gurobi(routes)
...
def solve_advanced(
    ...
    logging.info("--- Step 2: Advanced Optimization (Gurobi MIP) ---")
    ...
    logging.info(f"  2. Chi phí sau Gurobi MIP:    {final_int_cost}")
