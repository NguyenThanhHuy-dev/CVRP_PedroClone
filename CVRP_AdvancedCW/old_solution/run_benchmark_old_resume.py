...
# ============================================================================
# LẤY DANH SÁCH INSTANCE ĐÃ CHẠY ĐỂ RESUME
# ============================================================================
def get_processed_instances(csv_path: str) -> set:
    """ Đọc file CSV và trả về tập hợp (set) các instance đã tồn tại. """
    processed = set()
...
def run_benchmark(target_set: str, neighbor_num: int):
    log_filename = os.path.join(LOG_DIR, f"benchmark_{target_set}_old_resume_{time.strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
    ...
    # Lấy danh sách các bài đã chạy
    processed_instances = get_processed_instances(csv_file)

    files = sorted([f for f in os.listdir(instance_folder) if f.endswith(".vrp")])
    logging.info(f"=== BẮT ĐẦU BENCHMARK OLD SOTA (FERREIRA) - CHẾ ĐỘ RESUME | TẬP {target_set} ===")
    logging.info(f"Đã tìm thấy {len(processed_instances)} instance hoàn thành trong {csv_file}. Sẽ tiến hành bỏ qua.")
    ...
    for filename in files:
        ...
        # KIỂM TRA BỎ QUA NGAY TẠI ĐÂY
        if name in processed_instances:
            logging.info("-" * 60)
            logging.info(f"⏩ BỎ QUA: {name} (Đã tồn tại trong file CSV)")
            continue
