...
# -*- coding: UTF-8 -*-
"""
Advanced CVRP Optimizer using MaxSAT (Pairwise) + Gurobi (Single) + Timeout
===========================================================================
"""
...
# =====================================================================
# MAX-SAT OPTIMIZER (Chỉ giữ lại Pairwise Route Optimizer)
# =====================================================================
...
# =====================================================================
# MULTIPROCESSING WRAPPERS (Cho Pairwise)
# =====================================================================
...
class AdvancedCVRPOptimizer:
    def __init__(self, distances: np.ndarray, demands: np.ndarray, capacity: int, n_vehicles: int, config: Dict[str, Any] = None):
        ...
        # SỬ DỤNG GUROBI NÊN MAX_SINGLE_SIZE ĐƯỢC TĂNG LÊN RẤT LỚN
    ...
    def optimize_single_route_safe(self, route: List[int]) -> Tuple[List[int], int]:
        ...
        try:
            # GỌI GUROBI OPTIMIZER THAY VÌ MULTIPROCESSING
            ...
            logging.error("Lỗi: Không tìm thấy file 'gurobi_optimizer.py'. Vui lòng đảm bảo file đã được tạo.")
            return route, self.compute_route_cost(route)
        except Exception as e:
            logging.error(f"Lỗi khi chạy Gurobi: {e}")
            return route, self.compute_route_cost(route)
    ...
    def optimize(self, initial_routes: Dict[int, List[int]], max_iterations: int = 100) -> Tuple[Dict[int, List[int]], float]:
        ...
        logging.info("=========================================")
        logging.info("BẮT ĐẦU VÒNG LẶP OPTIMIZE (GUROBI + MAXSAT)")
        logging.info(f" -> Max Single Size  : {self.max_single_size} (Gurobi)")
        logging.info(f" -> Max Pairwise Size: {self.max_pairwise_size} (PySAT)")
...
def solve_advanced(filepath: str, config: Dict[str, Any] = None, max_iterations: int = 50):
    ...
    logging.info("--- Step 3: Advanced Optimization (Gurobi + MaxSAT) ---")
    ...
    logging.info(f"  3. Chi phí sau Gurobi+MaxSAT: {final_int_cost}")
    logging.info(f"  => TỔNG CẢI THIỆN:            {cw_cost - final_int_cost}")
    logging.info(f"  => Số lần Gurobi Single giúp giảm cost  : {stats['single_imp_count']} lần")
...
if __name__ == "__main__":
    ...
    test_config = {
        "max_single_size": 30,     # Gurobi cân được tuyến rất lớn
        "single_timeout": 5.0,
        "max_pairwise_size": 10,   # PySAT vẫn giữ 10
