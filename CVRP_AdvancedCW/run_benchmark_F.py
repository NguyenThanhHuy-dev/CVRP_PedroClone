...
# -*- coding: UTF-8 -*-
"""
Benchmark Runner for Fisher F-Set Instances
================================================
Sử dụng: python run_benchmark_F.py [METHOD]
...
# Thay đổi thư mục đích sang bộ F
INSTANCE_DIR = os.path.join("instances", "F")
RESULTS_DIR  = "results"
RESULT_FILE = os.path.join(RESULTS_DIR, f"benchmark_F_{METHOD}.csv")
...
def get_bks_from_sol(sol_filepath: str) -> int:
    """Đọc file .sol để lấy BKS (dòng chứa 'Cost')."""
...
def run_f_benchmark():
    ...
    # Lọc riêng các file bắt đầu bằng F-
    f_files = sorted(
        [f for f in os.listdir(INSTANCE_DIR) if f.startswith("F-") and f.endswith(".vrp")],
        key=lambda f: get_instance_info(f)[1]
    )

    if not f_files:
        print(f"[INFO] Không có file .vrp nào trong {INSTANCE_DIR}")
        return

    print("\n" + "=" * 70)
    print(f"BENCHMARK F-SET  |  METHOD: {METHOD.upper()}  |  {len(f_files)} instances")
    ...
    for idx, filename in enumerate(f_files):
        ...
        if existing_key in csv_data:
            ...
            print(f"\n[{idx+1:02d}/{len(f_files)}] {name} (N={n}, K={k}, BKS={bks}) ← đã chạy {old_runs} lần")
        else:
            print(f"\n[{idx+1:02d}/{len(f_files)}] {name} (N={n}, K={k}, BKS={bks}) ← lần đầu chạy")
