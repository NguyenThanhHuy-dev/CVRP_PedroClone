...
def force_merge_routes(routes: Dict[int, List[int]], distances: np.ndarray,
                       demands: np.ndarray, capacity: int, 
                       max_vehicles: int) -> Dict[int, List[int]]:
    """
    Gộp routes cho đến khi số routes <= max_vehicles.
    
    Chiến lược:
    1. Ưu tiên gộp 2 routes có tổng demand <= capacity
    2. Nếu không gộp được, di chuyển customers giữa các routes
    3. Nếu vẫn không được, ép gộp 2 routes nhỏ nhất (có thể vượt capacity tạm thời)
    """
    routes = {k: list(v) for k, v in routes.items() if v}  # Copy và loại bỏ routes rỗng
    
    max_iterations = 500
    iteration = 0
    
    while len(routes) > max_vehicles and iteration < max_iterations:
        iteration += 1
        route_ids = list(routes.keys())
        
        # Tính demand của từng route
        route_demands = {r: sum(demands[c] for c in routes[r]) for r in route_ids}
        
        # Chiến lược 1: Tìm cặp routes có thể gộp với tổng demand <= capacity
        best_merge = None
        best_cost_increase = float('inf')
        
        for i, r1 in enumerate(route_ids):
            for r2 in route_ids[i+1:]:
                total_demand = route_demands[r1] + route_demands[r2]
                if total_demand <= capacity:
                    min_link_cost = float('inf')
                    for c1 in routes[r1]:
                        for c2 in routes[r2]:
                            link_cost = distances[c1, c2]
                            if link_cost < min_link_cost:
                                min_link_cost = link_cost
                    
                    if min_link_cost < best_cost_increase:
                        best_cost_increase = min_link_cost
                        best_merge = (r1, r2)
        
        if best_merge:
            r1, r2 = best_merge
            merged = merge_two_routes(routes[r1], routes[r2], distances)
            routes[r1] = merged
            del routes[r2]
            print(f"    Merged routes {r1} and {r2} -> {len(merged)} customers")
            continue
        
        # Chiến lược 2: Di chuyển customers để tạo điều kiện gộp
        # Tìm route nhỏ nhất và thử di chuyển customers của nó sang routes khác
        sorted_routes = sorted(route_ids, key=lambda r: route_demands[r])
        moved_any = False
        
        for smallest_id in sorted_routes[:3]:  # Thử 3 routes nhỏ nhất
            customers = list(routes[smallest_id])
            
            for customer in customers:
                # Tìm route có thể nhận customer
                for target_id in route_ids:
                    if target_id == smallest_id:
                        continue
                    if route_demands[target_id] + demands[customer] <= capacity:
                        # Tìm vị trí chèn tốt nhất
                        target_route = routes[target_id]
                        best_pos = 0
                        best_cost = float('inf')
                        
                        for pos in range(len(target_route) + 1):
                            prev_node = target_route[pos - 1] if pos > 0 else 0
                            next_node = target_route[pos] if pos < len(target_route) else 0
                            cost = (distances[prev_node, customer] + 
                                   distances[customer, next_node] -
                                   distances[prev_node, next_node])
                            if cost < best_cost:
                                best_cost = cost
                        ...
                        # Di chuyển customer
                        routes[smallest_id].remove(customer)
                        routes[target_id].insert(best_pos, customer)
                        route_demands[target_id] += demands[customer]
                        route_demands[smallest_id] -= demands[customer]
                        moved_any = True
                        
                        # Nếu route rỗng, xóa nó
                        if not routes[smallest_id]:
                            del routes[smallest_id]
                            print(f"    Emptied route {smallest_id}")
                            break
                        break
                
                if smallest_id not in routes:
                    break
        ...
        # Chiến lược 3: Ép gộp 2 routes nhỏ nhất (có thể vượt capacity)
        print(f"    FORCED merge: No feasible merge found, merging 2 smallest routes")
        sorted_routes = sorted(route_ids, key=lambda r: route_demands[r])
        if len(sorted_routes) >= 2:
            ...
            new_demand = sum(demands[c] for c in merged)
            if new_demand > capacity:
                print(f"    WARNING: Merged route demand {new_demand} > capacity {capacity}")
    
    if len(routes) > max_vehicles:
        print(f"    WARNING: Could not reduce to {max_vehicles} routes, got {len(routes)}")
    
    # Renumber routes từ 0
    new_routes = {}
...
def merge_two_routes(route1: List[int], route2: List[int], 
                     distances: np.ndarray) -> List[int]:
    """
    Gộp 2 routes theo cách tốt nhất (4 cách: nối đầu-đầu, đầu-đuôi, đuôi-đầu, đuôi-đuôi).
    """
    if not route1:
        return route2
    if not route2:
    ...
    options = [
        # route1 + route2
        (route1 + route2, 
         distances[route1[-1], route2[0]]),
        # route1 + reversed(route2)
        (route1 + route2[::-1], 
         distances[route1[-1], route2[-1]]),
        # route2 + route1
        (route2 + route1, 
         distances[route2[-1], route1[0]]),
        # reversed(route2) + route1
        (route2[::-1] + route1, 
         distances[route2[0], route1[0]]),
    ]
    
    # Chọn cách gộp có link cost nhỏ nhất
    best_route, _ = min(options, key=lambda x: x[1])
    return best_route
...
def pairwise_optimize(routes: Dict[int, List[int]], distances: np.ndarray,
    ...
    while improved and iteration < max_iterations:
        ...
        for i, j in combinations(route_ids, 2):
            ...
            if len(routes[i]) + len(routes[j]) > 6:
                continue
...
def optimize_all_routes(routes: Dict[int, List[int]], distances: np.ndarray, 
                        demands: np.ndarray, capacity: int) -> Tuple[Dict[int, List[int]], int]:
    """
    Tối ưu tất cả routes bằng cách tối ưu từng route riêng lẻ.
    
    Args:
        routes: {vehicle_id: [customer_list]}
        distances: Distance matrix
        demands: Demand array
        capacity: Vehicle capacity
    
    Returns: (optimized_routes, total_cost)
    """
    print("\n=== Optimizing Individual Routes with MaxSAT ===")
    
    optimized = {}
    total_cost = 0
    
    for v, customers in routes.items():
        if len(customers) <= 1:
            # No optimization needed for single customer
            optimized[v] = customers
            if customers:
                cost = distances[0, customers[0]] + distances[customers[0], 0]
            else:
                cost = 0
            total_cost += cost
            print(f"  Route {v}: {customers} -> cost={cost} (no change)")
            continue
        
        # Calculate original cost
        original_cost = distances[0, customers[0]]
        for i in range(len(customers) - 1):
            original_cost += distances[customers[i], customers[i + 1]]
        original_cost += distances[customers[-1], 0]
        
        # Optimize this route
        optimizer = SingleRouteOptimizer(customers, distances)
        opt_route, opt_cost = optimizer.optimize()
        
        # Verify capacity
        demand = sum(demands[c] for c in opt_route)
        if demand > capacity:
            print(f"  Route {v}: CAPACITY VIOLATION! Using original.")
            optimized[v] = customers
            total_cost += original_cost
        else:
            optimized[v] = opt_route
            total_cost += opt_cost
            
            if opt_cost < original_cost:
                print(f"  Route {v}: {customers} -> {opt_route}")
                print(f"           cost: {original_cost} -> {opt_cost} (IMPROVED -{original_cost - opt_cost})")
            else:
                print(f"  Route {v}: cost={opt_cost} (no improvement)")
    
    return optimized, total_cost


def solve_with_clarke_wright_and_optimize(filepath: str):
    """
    ...
    # Step 1: Clarke-Wright (without reduce_routes which may fail)
    ...
    try:
        ...
        print(f"  Number of routes: {len(cw_routes)}")
        
        # Convert to our format
        routes = {i: list(route.value) for i, (_, route) in enumerate(cw_routes.items())}
        
    except Exception as e:
        print(f"  Clarke-Wright reduce_routes failed: {e}")
        print("  Using Clarke-Wright without route reduction...")
        
        # Run CW without reduce_routes
        cw = ClarkeWright(cvrp, n_vehicles)
        cw.load_savings()
        cw.load_routes()
        cw.combine_routes()
        # Skip reduce_routes
        ...
        print(f"  Time: {cw_time:.3f}s")
        print(f"  Cost: {cw_cost}")
        print(f"  Number of routes: {len(cw_routes)} (target: {n_vehicles})")
        
        # Convert to our format
        routes = {i: list(route.value) for i, (_, route) in enumerate(cw_routes.items())}
    
    # Step 1.5: Force merge if too many routes
    if len(routes) > n_vehicles:
        print(f"\n=== Step 1.5: Force Merge Routes ({len(routes)} -> {n_vehicles}) ===")
        routes = force_merge_routes(routes, cvrp.distances, demands, cvrp.capacity, n_vehicles)
        merge_cost = sum(calculate_route_cost(r, cvrp.distances) for r in routes.values())
        print(f"  After merge: {len(routes)} routes, cost = {merge_cost}")
        
        # Verify all customers are still served
        all_customers_in_routes = set()
        for r in routes.values():
            all_customers_in_routes.update(r)
        expected_customers = set(range(1, cvrp.dimension))
        if all_customers_in_routes != expected_customers:
            missing = expected_customers - all_customers_in_routes
            print(f"  WARNING: Missing customers after merge: {missing}")
    
    # Step 2: Two-Opt improvement (on merged routes)
    print("\n=== Step 2: Two-Opt Local Search ===")
    # Convert back to Route format for TwoOpt
    temp_routes = {}
    for v, customers in routes.items():
        r = Route(cvrp, customers)
        temp_routes[v] = r
    
    two_opt_time, two_opt_routes = TwoOpt.run(temp_routes)
