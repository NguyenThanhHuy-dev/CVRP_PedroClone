...
# -*- coding: UTF-8 -*-
"""
Benchmark Runner for Augerat A-Set Instances
=============================================
Sử dụng: python run_benchmark_A.py [METHOD]
...
INSTANCE_DIR = os.path.join("instances", "A")
RESULTS_DIR  = "results"
RESULT_FILE = os.path.join(RESULTS_DIR, f"benchmark_A_{METHOD}.csv")
...
def run_a_benchmark():
    ...
    a_files = sorted(
        [f for f in os.listdir(INSTANCE_DIR) if f.startswith("A-") and f.endswith(".vrp")],
        key=lambda f: get_instance_info(f)[1] 
    )

    if not a_files: return

    print("\n" + "=" * 70)
    print(f"BENCHMARK A-SET  |  METHOD: {METHOD.upper()}  |  {len(a_files)} instances")
    ...
    for idx, filename in enumerate(a_files):
        ...
        if existing_key in csv_data:
            ...
            print(f"\n[{idx+1:02d}/{len(a_files)}] {name} (N={n}, K={k}, BKS={bks}) ← đã có {old_runs} lần chạy, best={old_best}")
        else:
            print(f"\n[{idx+1:02d}/{len(a_files)}] {name} (N={n}, K={k}, BKS={bks}) ← lần đầu chạy")
