...
def build_dynamic_config(n: int, k: int) -> dict:
    ...
    if METHOD in ("gurobi", "cplex"):
        ...
        max_single   = 15
        max_pairwise = 16
        pair_timeout = min(60.0, max(60.0, avg * 4.0))
...
def run_b_benchmark():
    ...
    print("\n" + "=" * 70)
    print(f"BENCHMARK B-SET (RESUME MODE) | METHOD: {METHOD.upper()} | {len(b_files)} instances")
    ...
    for idx, filename in enumerate(b_files):
        ...
        # --- ĐIỂM THAY ĐỔI DUY NHẤT: THÊM LỆNH CONTINUE ĐỂ BỎ QUA ---
        if existing_key in csv_data:
            old_runs = csv_data[existing_key].get("Runs", "0")
            old_best = csv_data[existing_key].get("Best_Cost", "?")
            print(f"\n[{idx+1:02d}/{len(b_files)}] {name} (N={n}, K={k}, BKS={bks}) ← Đã hoàn thành (best={old_best}). BỎ QUA!")
            continue # Lệnh này sẽ lập tức nhảy sang vòng lặp tiếp theo
        else:
            print(f"\n[{idx+1:02d}/{len(b_files)}] {name} (N={n}, K={k}, BKS={bks}) ← Bắt đầu chạy...")
