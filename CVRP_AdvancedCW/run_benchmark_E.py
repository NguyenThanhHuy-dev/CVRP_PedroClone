...
# -*- coding: UTF-8 -*-
"""
Benchmark Runner for Augerat E-Set Instances
=============================================
Sử dụng: python run_benchmark_E.py [METHOD]
...
INSTANCE_DIR = os.path.join("instances", "E")
RESULTS_DIR  = "results"
RESULT_FILE  = os.path.join(RESULTS_DIR, f"benchmark_E_{METHOD}.csv")
...
def run_e_benchmark():
    ...
    e_files = sorted(
        [f for f in os.listdir(INSTANCE_DIR) if f.startswith("E-") and f.endswith(".vrp")],
        key=lambda f: get_instance_info(f)[1]
    )
    e_files = ["E-n31-k7.vrp"]
    if not e_files: return

    print("\n" + "=" * 70)
    print(f"BENCHMARK E-SET  |  METHOD: {METHOD.upper()}  |  {len(e_files)} instances")
    ...
    for idx, filename in enumerate(e_files):
        ...
        if existing_key in csv_data:
            ...
            print(f"\n[{idx+1:02d}/{len(e_files)}] {name} (N={n}, K={k}, BKS={bks}) ← đã có {old_runs} lần chạy, best={old_best}")
        else:
            print(f"\n[{idx+1:02d}/{len(e_files)}] {name} (N={n}, K={k}, BKS={bks}) ← lần đầu chạy")
