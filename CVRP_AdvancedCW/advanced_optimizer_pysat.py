...
class PairwiseRouteOptimizer:
    ...
    def optimize(self) -> Tuple[List[int], List[int], int, bool]:
        ...
        for lits in (lits_v1, lits_v0):
            cnf = PBEnc.leq(
                lits=lits, weights=weights, bound=self.cap, top_id=self.var_id
            )
            for clause in cnf.clauses:
                self.wcnf.append(clause)
            if cnf.clauses:
                self.var_id = max(
                    self.var_id, max(max(abs(l) for l in cl) for cl in cnf.clauses)
                )
...
class AdvancedCVRPOptimizer:
    ...
    # ------------------------------------------------------------------
    def _run_inner_loop(
        self, routes: Dict, max_iterations: int, outer_idx: int
    ...
    # ------------------------------------------------------------------
    def optimize(
        self, initial_routes: Dict, max_iterations: int = 100
    ) -> Tuple[Dict, float]:
        ...
        logging.info(
            f"  max_single={self.max_single_size}  s_to={self.single_timeout}s"
        )
        logging.info(
            f"  max_pair={self.max_pairwise_size}  p_to={self.pairwise_timeout}s"
        )
        logging.info(f"  n_pairs={self.n_closest_pairs}")
        logging.info(
            f"  inner_patience={self.inner_patience}  outer_patience={self.outer_patience}"
        )
        logging.info(
            f"  global_timeout={'∞' if self.global_timeout is None else f'{self.global_timeout}s'}"
        )
...
def solve_advanced(
    ...
    routes = {i: list(r.value) for i, (_, r) in enumerate(two_opt_routes.items())}
