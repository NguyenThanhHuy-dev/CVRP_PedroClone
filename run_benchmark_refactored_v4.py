...
def run_solver_wrapper(filepath):
    ...
    return route_optimizer_v4_refactored.solve_with_clarke_wright_and_optimize(
...
def main():
    ...
    # Setup output CSV
    result_file = (
        f"results_refactored_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    with open(result_file, mode="a", newline="", encoding="utf-8") as csv_file:
        ...
        for idx, data in enumerate(instances_list):
            ...
            if costs and min(costs) < float("inf"):
                ...
                print(
                    f"  📊 RESULT: Best={best_c} (Gap {gap_str}%) | AvgTime={avg_t:.2f}s | {valid_icon}"
                )
